...
def get_all_urls_from_sitemap():
    """Get ALL URLs from sitemap - both Turkish and English."""
    try:
        headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)'}
        all_urls = []
        ...
        filtered = []
        skip_extensions = ['.jpg', '.jpeg', '.png', '.gif', '.pdf', '.zip', '.doc']
        for url in all_urls:
            if any(ext in url.lower() for ext in skip_extensions):
                continue
            filtered.append(url)

        print(f"Total valid URLs: {len(filtered)}")
        return filtered
    except Exception as e:
        print(f"Error: {e}")
        return []
...
def get_english_urls_from_sitemap():
    """Get only English URLs from sitemap (original behavior)."""
    try:
        headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)'}
        all_turkish_urls = []
        for sitemap_url in SITEMAP_URLS:
...
def run_scraper():
    ...
    # Scrape ALL sitemap URLs (Turkish + English)
